...
{
    public function createAndPost(array $attributes, string $reservedQuantityToRelease = '0'): StockMovement
    {
        $attributes['number'] ??= 'MOV-' . now()->format('YmdHis') . '-' . strtoupper(Str::random(4));
        $attributes['status'] ??= 'draft';
        $attributes['performed_by'] ??= auth()->id();
        $movement = StockMovement::query()->create($attributes);
        return $this->post($movement, $reservedQuantityToRelease);
    }
    ...
    public function post(StockMovement $movement, string $reservedQuantityToRelease = '0'): StockMovement
    {
        return DB::transaction(function () use ($movement, $reservedQuantityToRelease): StockMovement {
