...
use App\Modules\Catalog\Domain\Models\{Brand,Product,ProductCategory,ProductClass,Unit}; use App\Modules\Foundation\Domain\Models\Tenant;
...
final class ProductResource extends Resource {
 ...
 Select::make('product_class_id')->label('Classe do produto')->options(fn()=>ProductClass::query()->where('status','active')->orderBy('name')->pluck('name','id'))->searchable()->preload()->required()->columnSpan(['default'=>1,'md'=>1,'xl'=>2]),
 ...
 Select::make('product_type')->label('Tipo')->options(['material'=>'Produto/Material','fuel'=>'Combustível','part'=>'Peça','epi'=>'EPI','asset'=>'Ativo imobilizado','input'=>'Insumo'])->default('material')->required(),
 ...
 public static function table(Table $table): Table { return $table->columns([TextColumn::make('code')->label('Código')->searchable()->sortable(),TextColumn::make('name')->label('Produto')->searchable()->sortable(),TextColumn::make('product_type')->label('Tipo')->badge()->formatStateUsing(fn (string $state) => match ($state){'material'=>'Produto/Material','fuel'=>'Combustível','part'=>'Peça','epi'=>'EPI','asset'=>'Ativo imobilizado','input'=>'Insumo',default => $state}),TextColumn::make('minimum_stock')->label('Estoque mínimo')->formatStateUsing(fn($state)=>number_format((float)$state,4,',','.')),IconColumn::make('track_stock')->label('Estoque')->boolean(),TextColumn::make('status')->label('Status')->badge()->formatStateUsing(fn (string $state) => $state === 'active' ? 'Ativo' : 'Inativo')->color(fn (string $state) => $state === 'active' ? 'success' : 'gray')])->recordActions([EditAction::make()->label('Editar')]); }
